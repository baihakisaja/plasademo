        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Antrian Informasi Product</h2>
...
<div class="reloadmodselesai3"></div>

         <div class="reloadmo3"></div>

<?php if($this->session->flashdata('berhasil_simpan') == TRUE): ?>
 <script type="text/javascript">
 swal("Berhasil!", "informasi Berhasil di SIMPAN", "success");
 </script>
<?php endif; ?>
...
<?php if($this->session->flashdata('berhasil_hapus') == TRUE): ?>
 <script type="text/javascript">
 swal("Berhasil!", "informasi Behasil di HAPUS", "success");
 </script>
<?php endif; ?>
...
<script>
    $(function () {
        ...
        const reloadTable3 = function () {
            $.ajax({
                    url   : '<?php echo site_url("informasi/list_informasi");?>',
                    type  : 'GET',
                    async : true,
                    dataType : 'json',
                        ...
                        for(i=0; i<data.length; i++){
                            html += '<tr>'+
                                    ...
                                    '<td style="text-align:right;">'+
                                        '<button type="button" class="btn btn-round btn-primary fa fa-pencil" data-toggle="modal" data-target="#ModalEdit'+ data[i].id_informasi +'"> Selesai</button>'+
                                        '<button type="button" class="btn btn-round btn-danger fa fa-trash" data-toggle="modal" data-target="#ModalHapus'+ data[i].id_informasi +'"> Hapus</button>'+
                                    '</td>'+
                                    '</tr>';
                        }
                        $('.informasi').html(html);
        ...
        const reloadmodhapus3 = function () {
            $.ajax({
                    url   : '<?php echo site_url("informasi/list_informasi");?>',
                    type  : 'GET',
                    async : true,
                    ...
                    success : function(data){
                        ...
                        var i;
                        for(i=0; i<data.length; i++){
                            html += '<div class="modal fade" id="ModalHapus'+ data[i].id_informasi +'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button><h4 class="modal-title" id="myModalLabel"> Hapus Antrian</h4></div><form class="form-horizontal" action="<?php echo base_url().'informasi/hapus_informasi'?> " method="post" enctype="multipart/form-data"><div class="modal-body">'+
            '<input type="hidden" name="kode" value="'+ data[i].id_informasi +'"/>'+
        ...
        const reloadmodselesai3 = function () {
            $.ajax({
                    url   : '<?php echo site_url("informasi/list_informasi");?>',
                    type  : 'GET',
                    async : true,
                    ...
                    success : function(data){
                        ...
                        var i;
                        for(i=0; i<data.length; i++){
                            html += '<div class="modal fade" id="ModalEdit'+ data[i].id_informasi +'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button><h4 class="modal-title" id="myModalLabel"> Selesai Pelayanan</h4></div>'+
    '<form class="form-horizontal" action="<?php echo base_url().'informasi/selesai'?>" method="post" enctype="multipart/form-data">'+
        '<div class="modal-body"><input value="'+ data[i].id_informasi +'" name="id_informasi" type="hidden"><input type="hidden" name="kode" value="'+ data[i].id_informasi +'"/>'+
