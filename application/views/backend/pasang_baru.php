        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Antrian Registrasi Prioritas </h2>
...
<div class="reloadmodselesai"></div>

         <div class="reloadmo"></div>

<?php if($this->session->flashdata('berhasil_simpan') == TRUE): ?>
 <script type="text/javascript">
 swal("Berhasil!", "pasang_baru Berhasil di SIMPAN", "success");
 </script>
<?php endif; ?>
...
<?php if($this->session->flashdata('berhasil_hapus') == TRUE): ?>
 <script type="text/javascript">
 swal("Berhasil!", "pasang_baru Behasil di HAPUS", "success");
 </script>
<?php endif; ?>
...
<script>
    $(function () {
        ...
        const pasang_baruTable = $('#pasang_baru').DataTable();
        const reloadTable = function () {
            $.ajax({
                    url   : '<?php echo site_url("pasang_baru/list_pasang_baru");?>',
                    type  : 'GET',
                    async : true,
                    dataType : 'json',
                    success : function(data){
                        ...
                        for(i=0; i<data.length; i++){
                            html += '<tr>'+
                                    ...
                                    '<td>'+ data[i].nomor_pelanggan +'</td>'+
                                    '<td>'+ data[i].jam +'</td>'+
                                    '<td>'+ data[i].tgl +'</td>'+
                                    '<td>'+ data[i].nama_daerah +'</td>'+
                                    '<td style="text-align:right;">'+
                                        '<button type="button" class="btn btn-round btn-primary fa fa-pencil" data-toggle="modal" data-target="#ModalEdit'+ data[i].id_pasang_baru +'"> Selesai</button>'+
                                        '<button type="button" class="btn btn-round btn-danger fa fa-trash" data-toggle="modal" data-target="#ModalHapus'+ data[i].id_pasang_baru +'"> Hapus</button>'+
                                    '</td>'+
                                    '</tr>';
                        }
                        $('.pasang_baru').html(html);
                    }
 
                });
        }
        const reloadmodhapus = function () {
            $.ajax({
                    url   : '<?php echo site_url("pasang_baru/list_pasang_baru");?>',
                    type  : 'GET',
                    async : true,
                    ...
                    success : function(data){
                        ...
                        var i;
                        for(i=0; i<data.length; i++){
                            html += '<div class="modal fade" id="ModalHapus'+ data[i].id_pasang_baru +'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button><h4 class="modal-title" id="myModalLabel"> Hapus Antrian</h4></div><form class="form-horizontal" action="<?php echo base_url().'pasang_baru/hapus_pasang_baru'?> " method="post" enctype="multipart/form-data"><div class="modal-body">'+
            '<input type="hidden" name="kode" value="'+ data[i].id_pasang_baru +'"/>'+
        ...
        const reloadmodselesai = function () {
            $.ajax({
                    url   : '<?php echo site_url("pasang_baru/list_pasang_baru");?>',
                    type  : 'GET',
                    async : true,
                    ...
                    success : function(data){
                        ...
                        var i;
                        for(i=0; i<data.length; i++){
                            html += '<div class="modal fade" id="ModalEdit'+ data[i].id_pasang_baru +'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button><h4 class="modal-title" id="myModalLabel"> Selesai Pelayanan</h4></div>'+
    '<form class="form-horizontal" action="<?php echo base_url().'pasang_baru/selesai'?>" method="post" enctype="multipart/form-data">'+
        '<div class="modal-body"><input value="'+ data[i].id_pasang_baru +'" name="id_pasang_baru" type="hidden"><input type="hidden" name="kode" value="'+ data[i].id_pasang_baru +'"/>'+
